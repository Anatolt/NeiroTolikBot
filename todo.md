...
- [ ] Отрефакторить services/memory.py: разнести по модулям (messages/settings/voice/admin) и сделать единый слой работы с БД
- [ ] При /set_voice_model показывать текущую выбранную модель
- [ ] Локальный TTS-генератор для автопроверки: бот заходит в голос, говорит тестовую фразу, проверяет, что транскрипция приходит (или найти сервера с постоянным общением)
- [ ] Писать админу, что закончились токены (API quota)
- [ ] Переделать бота с полинга на вебхуки
- [ ] Логировать путь к memory.db и количество flows при старте, чтобы ловить моменты очистки/подмены БД
- [x] Discord уведомления о входе в голос: не писать "0 чел"; писать "в чате пока больше никого", а при >=1 перечислять людей
...
- [x] Проверить, не передается ли системный промпт в генерацию картинки
- [x] Сделать, чтобы был виден промпт для генерации картинки
...
- [ ] возмонжость выбора модели для роутинга запросов (сейчас только gpt 4)
...
- [x] Ограничение длины голосовых сообщений
...
- [x] Ответ на голосовуху в группе через тег бота
- [ ] Ответ голосовыми сообщениями
- [x] Показывать оценку стоимости распознавания для длинных голосовух
...
- [ ] Саммари при отправке в Telegram должно быть ровно в одно сообщение, без Markdown, не обрезаться
- [x] Звуковые дорожки для распознавания обрезаются слишком рано — мешает распознаванию
- [ ] Проверить почему отправляются звуковые чанки больше 10 секунд
- [ ] Режимы записи: слушать все каналы сразу и отправлять один файл / слушать каждого участника отдельно (как сейчас)
- [ ] Файлы debug-записей называть по шаблону `имя_пользователя+tmp...`
- [ ] Отдельная команда для включения/отключения получения звуковых отрезков
- [ ] Если STT (локальный/облачный) недоступен — не удалять записи, копить до лимита диска, мониторить свободное место, слать алерты админам
- [x] Доработать алгоритм нарезки аудио для распознавания (временный raw-режим)
...
- [ ] В дневных саммари отмечать, если за день было несколько разных диалогов с большими паузами
- [ ] Убрать реакцию на слова без слеш-команд
- [ ] Убрать кусок кода, который выдает перечень моделей без пагинации
- [ ] Единый список команд: подключить в README, /help, системный контекст и selftest
- [ ] Голосовая команда: проиграть последний записанный кусочек аудио для проверки связи
- [ ] Команда: добавить фичу в TODO по запросу пользователя
- [ ] В вывод моделей добавить объём контекста
- [ ] В вывод моделей добавить сортировки по объёму контекста и другим параметрам
- [ ] Команда: показать всё, что отправляется в LLM API (полный запрос)
- [ ] Talker: рассказывать каждый раз новый анекдот; вести базу фраз, при исчерпании автоматически генерировать новые; говорить разными голосами (мужской/женский, высокий/низкий)
...
- [ ] В ответе от LLM-роутера указывать, в какую модель пойдёт запрос
...
- [ ] Реакция на ключевое слово ботик и варианты (бодик, батик); собрать другие частые варианты распознавания
- [ ] Вынести генерацию изображений в отдельный модуль
- [ ] Разделить остальные функции по модулям (Shared Actions): вынести бизнес-логику из обработчиков Telegram/Discord в единый слой
- [ ] Отрефакторить services/memory.py: разнести по модулям (db_client.py, chat_history.py, user_profile.py, voice_data.py, kv_store.py)
- [x] Зарефакторить `discord_bot.py` — файл слишком большой
- [ ] Перейти с глобального словаря BOT_CONFIG на структурированные конфиги (Pydantic или Dataclasses)
- [ ] Использовать менеджер процессов (например, Supervisor) вместо start-bot.sh для контроля за tbot.py и discord_bot.py
- [ ] Внедрить стандарт использования `ctx.defer()` и таймаутов для всех "тяжелых" команд в Discord (по аналогии с /join)
...
- [ ] Миниапп: добавить настройку показа техшапки (заголовка ответа)
- [ ] Миниапп: добавить настройку переспрашивания подтверждения /yes в LLM-роутинге
- [ ] Миниапп: добавить настройку режима роутинга (rules/llm)
- [ ] Миниапп: показывать доступные API и что доступно внутри каждого (OpenRouter/OpenAI/PiAPI/ImageRouter и др.)
- [ ] Миниапп: продумать удобный UI для показа доступности по каждому API

- [ ] Добавить в техшапку информацию о времени API-запроса (от отправки до получения ответа)
- [ ] Добавить ачивки за использование 10, 20, 50, 100 разных моделей
- [ ] Добавить дейстрик (streak) пользования ботом

- [ ] Генерация музыки через API Suno
- [ ] Генерация и проигрывание музыки в голосовом канале Discord по голосовым командам или по /sing
